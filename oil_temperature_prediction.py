...
def plot_results(dates, actuals, predictions):
    num_points_to_plot = min(len(dates), len(actuals), len(predictions))  # Get minimum length

    plt.figure(figsize=(16, 8))
    plt.plot(dates[-num_points_to_plot:], actuals[-num_points_to_plot:], label='Actual', color='blue')  # Slice data for plotting
    plt.plot(dates[-num_points_to_plot:], predictions[-num_points_to_plot:], label='Predicted', color='red', alpha=0.7)  # Slice data for plotting
    plt.title('Oil Temperature: Actual vs Predicted')
    plt.xlabel('Date')
...
predictions, actuals, prediction_len = evaluate_model(model, test_loader, scaler, device)  # Get prediction_len

# Use the correct time range from the test dataset for plotting (Modified)
test_dates = df.index[-prediction_len:]  # Use prediction_len to slice the index

# Plot the results using the adjusted test_dates
